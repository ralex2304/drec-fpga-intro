...
always @(*) begin
    case (opcode)
        ...
        BRANCH:  {o_sel1, o_sel2, o_sel_wb} = {IMM_B, PC   , WB_ALU  };
...
always @(*) begin
    ...
    if (opcode == OP_IMM && funct3 == 3'b000) begin // addi
        o_alu_oper[3] = 1'b0;
    end else if (opcode == LOAD || opcode == STORE || opcode == BRANCH || opcode == AUIPC) begin
        o_alu_oper = 4'b0000;
    end
end
...
assign o_jump   = (opcode == JALR) || (opcode == JAL);

assign o_mem_wr_en = (opcode == STORE);
