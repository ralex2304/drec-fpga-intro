...
localparam STAGE1_PIPE_LEN = 1;

reg sum_s_pipe [STAGE1_PIPE_LEN:0];
reg [MLEN*2+1:0] sum_abs_m_pipe [STAGE1_PIPE_LEN:0];

always @(*) begin
    sum_s_pipe[0]     = sum_signed_m[MLEN*2+2];
    sum_abs_m_pipe[0] = sum_s_pipe[0] ? -sum_signed_m[MLEN*2+1:0] : sum_signed_m[MLEN*2+1:0];
end

reg [ELEN-1:0] m_shift_pipe [STAGE1_PIPE_LEN:0];
reg m_one_found_pipe [STAGE1_PIPE_LEN:0];
integer m_shift_i;
always @(*) begin
    m_shift_pipe[0] = {ELEN{1'b0}};
    m_one_found_pipe[0] = 1'b0;
    for (m_shift_i = MLEN*2+1; m_shift_i >= 0; m_shift_i = m_shift_i - 1) begin
        if (sum_abs_m_pipe[0][MLEN*2+1 - m_shift_i]) begin
            m_shift_pipe[0] = m_shift_i[ELEN-1:0];
            m_one_found_pipe[0] = 1'b1;
        end
    end
end
...
generate
genvar s1_i;
for (s1_i = 0; s1_i < STAGE1_PIPE_LEN; s1_i = s1_i + 1) begin : genStage1Pipe
    always @(posedge clk) begin
        m_shift_pipe    [s1_i + 1] <= m_shift_pipe    [s1_i];
        m_one_found_pipe[s1_i + 1] <= m_one_found_pipe[s1_i];
        sum_abs_m_pipe  [s1_i + 1] <= sum_abs_m_pipe  [s1_i];
        sum_s_pipe      [s1_i + 1] <= sum_s_pipe      [s1_i];
    end
end
endgenerate
...
wire   [ELEN-1:0] m_shift     = m_shift_pipe    [STAGE1_PIPE_LEN];
wire              m_one_found = m_one_found_pipe[STAGE1_PIPE_LEN];
wire [MLEN*2+1:0] sum_abs_m   = sum_abs_m_pipe  [STAGE1_PIPE_LEN];
wire              sum_s       = sum_s_pipe      [STAGE1_PIPE_LEN];
